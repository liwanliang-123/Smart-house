...
{
	//将温湿度写入Socket
	write(c_fd,strff_TMP,strlen(strff_TMP));
	sleep(2);
	write(c_fd,strdd_RH,strlen(strdd_RH));
}
...
int main(void)
{
	...
	//温湿度线程
	int             temH     = pthread_creat(&temthread,NULL,tem_thread,NULL);
	if(temH == 0){

		printf("tem creat success!");
	}

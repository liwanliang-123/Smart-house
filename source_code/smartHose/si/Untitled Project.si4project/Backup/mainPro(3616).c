...
void *voice_thread(void *data)
{
	...
	while(1){
		...
		if(strstr(voicdHeadler->command,"KCL") != NULL){           //开厨房灯
				printf("KELD>>>>>>>>>>>>\n");
        ...
        }else if(strstr(voicdHeadler->command,"GCL") != NULL){           //关二楼灯
        		printf("GELD>>>>>>>>>>>>\n");
				tmp = findDeviceByname("RestaurantLight",pdeviceHead);      //找二楼的链表节点
				tmp->devicesInit(tmp->pinNum);                     
				tmp->close(tmp->pinNum);
		...
		if(strstr(voicdHeadler->command,"KCT") != NULL){           //开二楼灯
				printf("KELD>>>>>>>>>>>>\n");
				tmp = findDeviceByname("upstairLight",pdeviceHead);      //找二楼的链表节点
				tmp->devicesInit(tmp->pinNum);                     
				tmp->open(tmp->pinNum);
        ...
        }else if(strstr(voicdHeadler->command,"GCT") != NULL){           //关二楼灯
        		printf("GELD>>>>>>>>>>>>\n");
				tmp = findDeviceByname("upstairLight",pdeviceHead);      //找二楼的链表节点
				tmp->devicesInit(tmp->pinNum);                     
				tmp->close(tmp->pinNum);

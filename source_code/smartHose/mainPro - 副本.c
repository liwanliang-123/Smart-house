...
void *voice_thread(void *data)
{
	...
	while(1){
		...
		switch(voicdHeadler->command)
		{
			case KEL:if(strstr(voicdHeadler->command,"KEL") != NULL)           //开二楼灯
					printf("KELD>>>>>>>>>>>>\n");
			...
			else if(strstr(voicdHeadler->command,"WHR") != NULL){           //开餐厅灯
					printf("KELD>>>>>>>>>>>>\n");
	        ...
	        }else if(strstr(voicdHeadler->command,"WHL") != NULL){           //关二楼灯
	        		printf("GELD>>>>>>>>>>>>\n");
					tmp = findDeviceByname("RestaurantLight",pdeviceHead);      //找二楼的链表节点
					tmp->devicesInit(tmp->pinNum);                     
					tmp->close(tmp->pinNum);
			...
			else if(strstr(voicdHeadler->command,"KWS") != NULL){           //开卧室灯
					printf("KELD>>>>>>>>>>>>\n");
	        ...
	        }else if(strstr(voicdHeadler->command,"GWS") != NULL){           //关二楼灯
	        		printf("GELD>>>>>>>>>>>>\n");
					tmp = findDeviceByname("bathRoomLight",pdeviceHead);      //找二楼的链表节点
					tmp->devicesInit(tmp->pinNum);                     
					tmp->close(tmp->pinNum);
			...
			else if(strstr(voicdHeadler->command,"KCF") != NULL){           //开二楼灯
					printf("KELD>>>>>>>>>>>>\n");
	        ...
	        }else if(strstr(voicdHeadler->command,"GCF") != NULL){           //关二楼灯
	        		printf("GELD>>>>>>>>>>>>\n");
					tmp = findDeviceByname("livingRoomLight",pdeviceHead);      //找二楼的链表节点
					tmp->devicesInit(tmp->pinNum);                     
					tmp->close(tmp->pinNum);
...
void *camera_thread(void *data)
{
	...
	while(1){
		...
		sleep(2);
		if(dis >= camera_data){											  //低电平表示有人靠近
			printf("camera waiting....\n");
			sleep(2);
		}else if(dis < camera_data){                                         //距离大于25
